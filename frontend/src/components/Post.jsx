...
const Post = ({ post }) => {
      ...
      const handleLike = async () => {
        try {
          const res = await axios.get(`${import.meta.env.VITE_BASE_URL}/api/post/like/${post._id}`,{withCredentials:true});
      ...
      <div className="w-full h-20 justify-between items-center px-2.5 flex">
       ...
       <div className="w-full h-[60px]  flex justify-between items-center px-5 mt-2.5">
        <div className="flex justify-center items-center gap-2.5">
          <div className="flex justify-center items-center gap-[5px]" onClick={handleLike}>{post?.likes?.includes(userData._id) ? <GoHeartFill className="w-[25px] h-[25px] cursor-pointer text-red-600"/> : <GoHeart className="w-[25px] h-[25px] cursor-pointer"/>}
        ...
        <div>
          {userData.saved.includes(post?._id) ? <GoBookmarkFill className="w-[25px] h-[25px] cursor-pointer"/>:<MdOutlineBookmarkBorder className="w-[25px] h-[25px] cursor-pointer"/>}
        </div>
       </div>

       {
        post?.caption && <div className="w-full pb-4 px-5 gap-2.5 flex justify-start items-center">
       ...
       {
        showComment && <div className="w-full flex flex-col gap-[30px] pb-5">
          ...
          <input type="text" className="px-2.5 border-b-2 border-b-gray-500 w-[90%] outline-none h-10 placeholder:text-gray-500" placeholder="Write Comment..."/>
          <button className="absolute right-5 cursor-pointer"><IoSendSharp/></button>
          </div>
        </div>
       }
    </div>
