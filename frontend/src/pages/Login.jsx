...
const Login = () => {
  ...
  const loginHandler = async (e)=>{
    ...
    e.preventDefault();
    try {
      const res = await axios.post(`${import.meta.env.VITE_BASE_URL}/api/auth/login`,formData,{withCredentials:true});
  ...
  return (
    <div className="w-full h-screen bg-linear-to-b from-black to-gray-900 flex flex-col justify-center items-center">
      <div className="w-[90%] lg:max-w-[60%] h-[600px] bg-white rounded-2xl flex justify-center items-center overflow-hidden border-2 border-[#1a1f23]">
        {/* left container */}
        <form onSubmit={loginHandler} className="w-full justify-center lg:w-[50%] h-full bg-white flex flex-col items-center p-2.5 gap-5">
          ...
          <div className="w-[90%] px-5 cursor-pointer text-blue-700" onClick={()=>navigate("/forgot-password")}>Forgot Password?</div>

            {err && <p className="text-red-500">{err}</p>}
          <button type="submit" disabled={loading} className="w-[70%] px-5 py-2.5 bg-black text-white font-semibold h-[50px] cursor-pointer rounded-2xl mt-[30px] hover:bg-gray-900 transition-all">{loading ? <ClipLoader size={30} color="white"/> : "Sign In"}</button>
          <p className="cursor-pointer text-gray-800" onClick={()=>navigate('/signup')}>Want TO Create A New Account? <span className="text-blue-700 border-b-2 border-b-blue-700 pb-[3px]">Sign Up</span></p>
        </form>
        {/* right container */}
